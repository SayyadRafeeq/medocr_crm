...
const customIcon = L.icon({
  iconUrl: "/public/images/map-pin.svg",
  iconSize: [40, 40],
  iconAnchor: [15, 30],
  popupAnchor: [0, -30],
});
...
$(document).ready(function () {
  ...
  function showLocation(position) {
    ...
    L.marker([currentLat, currentLon], { icon: customIcon }).addTo(map);

    // Add built-in locate control
    L.control
      .locate({
        position: "topleft",
        setView: true,
              ...
              <div class="px-4 py-3 suggestion-detail">
                <div class="h-32 w-full bg-cover bg-center" 
                    style="background-image: url('${
                      place.imageUrl || "/public/images/hospital.svg"
                    }')">
                </div>
                <div class="flex flex-col gap-2 mt-2">
                  ...
                  <p class="flex gap-2 items-center"> 4.3 <span class="text-xl text-star-orange">${
                    avgRating ? stars + ` (${avgRating})` : stars
                  }</span></p>
                ...
                <div class="tab-content" id="best">
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex flex-col items-center gap-3">
                      <input type="text" placeholder="Your Location" class="${
                        selectedColor === "dark-blue"
                          ? "text-white placeholder:text-white"
                          : ""
                      } backdrop-blur-[20px] shadow-[4px_4px_10px_2px_#00000026] text-sm px-3 py-1.5 rounded-md w-full border-none focus:outline-none focus:ring-0" style="background-color: ${bgColor}A6;"/>
                      <input type="text" placeholder="Zenith Hospital" class="${
                        selectedColor === "dark-blue"
                          ? "text-white placeholder:text-white"
                          : ""
                      } backdrop-blur-[20px] shadow-[4px_4px_10px_2px_#00000026] text-sm px-3 py-1.5 rounded-md w-full border-none focus:outline-none focus:ring-0" style="background-color: ${bgColor}A6;" />
    ...
    if (matches.length > 0) {
      matches.forEach((place) => {
        ...
        const $item = $(`
          <div class="px-4 py-3 border-b border-${selectedColor} last:border-b-0 cursor-pointer transition-all hover:bg-opacity-10">
            <div class="flex flex-col gap-2">
              ...
              <p>Hospital</p>
              <p class="text-sm">${
                place.address || "Flat No. 7, 11th Floor, Chembur, Mumbai"
              }</p>
            ...
            <div class="ml-4 flex justify-between mt-2">
              <button class="flex flex-col gap-2 text-xs cursor-pointer" data-lat="${
                place.lat
              }" data-lon="${place.lon}">
              ...
              <a href="tel:${
                place.phone
              }" title="Call" class="flex flex-col gap-2 text-xs cursor-pointer">
                <span class='material-symbols-outlined material-filled !text-xl !font-semibold'>call</span><span>Call</span></a>
              <button class="flex flex-col gap-2 text-xs cursor-pointer" data-name="${encodeURIComponent(
                place.name
              )}">
        ...
        $item.on("click", function () {
          ...
          const overviewHtml = `
            <div class="px-4 py-3 suggestion-detail">
              ...
              <div class="tab-content" id="overview">
                <div class="ml-4 flex justify-between mt-2">
                  <button class="flex flex-col gap-2 text-xs cursor-pointer overviewDirections" data-lat="${
                    place.lat
                  }" data-lon="${place.lon}">
                    <span class='material-symbols-outlined material-filled !text-xl !font-semibold'>directions</span><span>Directions</span>
                  </button>
          ...
          $suggestionBox.on("click", "#overviewShare", function () {
            const url = `${window.location.origin}?place=${$(this).data(
              "name"
            )}`;
      ...
      $suggestionBox.prepend(`<div class='flex justify-between p-2 border-b border-${selectedColor}'>
        ...
        <div class="relative dropdown inline-block">
               <button class="filterToggle  cursor-pointer">
                <span class="material-symbols-outlined">swap_vert</span>
                </button>
                <div class="filterDropdown absolute right-0 mt-2 bg-white text-dark-gray shadow-[4px_4px_10px_2px_#00000026] hidden rounded-md w-22 z-10">
                  <div
                    class="trigger-visit p-2 pl-4 hover:bg-dodger-blue hover:text-white rounded cursor-pointer flex justify-between items-center">
                      Sort by
                  </div>
                  <div
                    class="trigger-status p-2 pl-4 hover:bg-dodger-blue hover:text-white rounded cursor-pointer flex justify-between items-center">
                      Services
                  </div>
                </div>
                <div id="statusSubmenu"
                  class="submenu hidden  absolute top-10 right-22 z-50 bg-white shadow-md rounded w-50 border border-gray-200">
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Ambulance Service</span> 
                  </div>
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Dental Services</span> 
                  </div>
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Cancer</span> 
                  </div>
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Emergency Care</span> 
                  </div>
                </div>
                <div id="visitSubmenu"
                  class="submenu hidden absolute top-10 right-22 z-50 bg-white shadow-md rounded w-40 border border-gray-200">
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Open</span> 
                  </div>
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Closed</span> 
                  </div>
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Distance</span> 
                  </div>
                </div>
            </div>      </div>`);
      $suggestionBox.show();
  ...
  function showFilteredSuggestions(title, filterFn) {
          ...
          <span>${title}</span>
            <div class="relative dropdown inline-block">
               <button class="filterToggle  cursor-pointer">
                ...
                <div class="filterDropdown absolute right-0 mt-2 bg-white text-dark-gray shadow-[4px_4px_10px_2px_#00000026] hidden rounded-md w-44 z-10">
                  <div
                    class="trigger-visit p-2 pl-4 hover:bg-dodger-blue hover:text-white rounded cursor-pointer flex justify-between items-center">
                      Sort by
                  </div>
                ...
                <div id="statusSubmenu"
                  class="submenu hidden  absolute top-10 right-44 z-50 bg-white shadow-md rounded w-50 border border-gray-200">
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Ambulance Service</span> 
                ...
                <div id="visitSubmenu"
                  class="submenu hidden absolute top-10 right-44 z-50 bg-white shadow-md rounded w-40 border border-gray-200">
                  <div
                    class="p-2 pl-4 hover:bg-dodger-blue  cursor-pointer flex items-center gap-2 group">
                    <span class="material-symbols-outlined text-light-gray !text-xl">check</span>
                    <span class="font-normal text-base text-dark-gray group-hover:text-white">Open</span> 
    ...
    filteredPlaces.forEach((place) => {
      ...
      const $item =
        $(`<div class="px-4 py-3 border-b border-${selectedColor} cursor-pointer transition-all hover:bg-opacity-10">
          <div class="flex flex-col gap-2">
            <p class="text-lg font-semibold">${place.name}</p>
            <p class="flex gap-2 items-center"><span class="text-xl text-star-orange">${
              avgRating ? stars + ` (${avgRating})` : stars
            }</span></p>
  ...
  $(document).ready(function () {
    // Remove all existing filter dropdown code and use only this:

    // 1. Toggle Main Dropdown
    $(document).on("click", ".filterToggle", function (e) {
      e.stopPropagation();
      const $button = $(this);
      const $dropdown = $button.siblings(".filterDropdown");
    ...
    $(document).on("click", ".trigger-date", function (e) {
      ...
      // Open date submenu
      $("#dateSubmenu").removeClass("hidden");
    });

    // 3. Open Status Submenu
    $(document).on("click", ".trigger-status", function (e) {
      e.stopPropagation();

      // Close other submenus
      $(".submenu").not("#statusSubmenu").addClass("hidden");
      $("#calendarContainer").addClass("hidden");

      // Open status submenu
      $("#statusSubmenu").removeClass("hidden");
    });

    // 4. Open Visit Type Submenu
    $(document).on("click", ".trigger-visit", function (e) {
      e.stopPropagation();

      // Close other submenus
      $(".submenu").not("#visitSubmenu").addClass("hidden");
      $("#calendarContainer").addClass("hidden");

      // Open visit submenu
      $("#visitSubmenu").removeClass("hidden");
    });

    // 5. Open Calendar when clicking "Custom"
    $(document).on("click", ".trigger-custom", function (e) {
      ...
      if (!$(".datepicker-inline").hasClass("hasDatepicker")) {
        $(".datepicker-inline").datepicker({
          onSelect: function (dateText) {
            console.log("Selected date: " + dateText);

            // Mark Custom option as selected
            $(".trigger-custom .material-symbols-outlined")
              .first()
    ...
    // 6. Handle option selection in Date submenu (Week/Month only)
    $(document).on(
      "click",
      "#dateSubmenu > div:not(.trigger-custom)",
      function (e) {
        e.stopPropagation();
    ...
    // 7. Handle option selection in Status submenu
    $(document).on("click", "#statusSubmenu > div", function (e) {
      e.stopPropagation();

      // Remove active state from all options in status submenu
    ...
    // 8. Handle option selection in Visit Type submenu
    $(document).on("click", "#visitSubmenu > div", function (e) {
      e.stopPropagation();

      // Remove active state from all options in visit submenu
      $("#visitSubmenu .material-symbols-outlined")
        .removeClass("!text-dodger-blue")
        .addClass("text-light-gray");
    ...
    // 9. Global Close - Close dropdowns when clicking outside
    $(document).on("click", function (e) {
      if (
        !$(e.target).closest(
          ".filterToggle, .filterDropdown, .submenu, #calendarContainer"
        ).length
      ) {
        $(".filterDropdown").addClass("hidden");
        $(".submenu").addClass("hidden");
    ...
    // 10. Prevent dropdown from closing when clicking inside
    $(document).on(
      "click",
      ".filterDropdown, .submenu, #calendarContainer",
      function (e) {
        e.stopPropagation();
      }
    );
