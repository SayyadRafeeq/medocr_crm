$(document).ready(function () {
  ...
  $(document).on("click", ".complete-btn", function () {
    $(".enquiredDetail").hide();
    $(".completedDetails").removeClass("hidden");
  });

  let uploadedFile = null;

  // Trigger file input when upload section is clicked
  $(document).on("click", ".uploadSection", function (e) {
    e.preventDefault();
    e.stopPropagation();

    // Only trigger if no file is uploaded yet
    if (!uploadedFile) {
      $("#fileInput")[0].click();
    }
  });
  ...
  $(document).on("change", "#fileInput", function (e) {
    ...
    if (file) {
      ...
      // Hide the upload text and icon, show filename with close icon
      $(".uploadSection").html(`
        <div class="flex items-center gap-2.5 bg-pale-azure rounded-lg py-2 px-4">
          <span class="font-normal text-sm text-primary-blue">${file.name}</span>
          <span class="material-symbols-outlined text-strong-red cursor-pointer  closeFile">close</span>
        </div>
      `);
    }
  });
  ...
  // Handle file removal
  $(document).on("click", ".closeFile", function (e) {
    e.preventDefault();
    e.stopPropagation();
    ...
    $(".uploadSection").html(`
      <span class="font-semibold text-sm text-spanish-gray">Upload</span>
      <span class="material-symbols-outlined text-spanish-gray">upload</span>
      <input type="file" id="fileInput" class="hidden" accept=".pdf,.doc,.docx,.jpg,.png">
    `);
  });
});
